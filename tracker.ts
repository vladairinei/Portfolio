...
function calcNightMinutes(start: number, end: number, pauseMin: number): number {
  ...
  for (let t = start; t < end; t++) {
    if (t >= pauseStart && t < pauseEnd) continue; // skip pause minutes
...
(function initMonthYearToToday() {
  ...
  if (!yearPicker.value) yearPicker.value = y;
  if (!monthOnly.value) monthOnly.value = m;
...
function renderHistory(): void {
  if (!historyTable) return; // no table in calendar mode
...
function renderCalendar(): void {
    ...
    // Ensure selectors are valid; prevents falling back to January
    if (!yearPicker.value) yearPicker.value = String(new Date().getFullYear());
